...
PACKAGE_ROOT = Path(os.path.abspath(os.path.dirname(__file__))).parent.parent   #these 2 line of codes ensure that we are able to load this config file without any error.
...
def load_dataset(file_name):
    ...
    return _data[config.FEATURES] #include only selected columns
...
def load_pipeline(pipeline_to_load):
    ...
    model_loaded = joblib.load(save_path)# Deserialize the pipeline from the specified pat
